...
type GatewayAuth = { orgId: string; userId: string };
...
function serializeErrorForLogs(error: unknown): Record<string, unknown> {
...
function systemAuth(orgId: string): GatewayAuth {
...
function gatewayHeaders(key: string, auth: GatewayAuth): Record<string, string> {
  return {
...
async function fetchObjectBytesFromGateway(
  objectKey: string,
  auth: GatewayAuth,
): Promise<ArrayBuffer> {
...
export async function indexDocumentInQdrant(input: {
  orgId: string;
  uploadId: string;
  dealId: string;
  docType: string;
...
export const ENHANCED_CLASSIFICATION_RULES: ReadonlyArray<{
  pattern: RegExp;
  docType: DocType;
  confidence: number;
}> = [
...
export function classifyByFilename(filename: string): { docType: DocType; confidence: number } {
