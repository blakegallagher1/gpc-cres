...
export async function getToolHealth(orgId: string) {
  ...
  return Array.from(byTool.values()).map((tool) => {
    const successRate =
      tool.totalCalls > 0 ? (tool.successCount / tool.totalCalls) * 100 : 100;
    const fallbackRate = tool.totalCalls > 0 ? tool.fallbackCount / tool.totalCalls : 0;
    const avgLatency =
      tool.totalCalls > 0 ? Math.round(tool.totalLatency / tool.totalCalls) : 0;

    let status: "HEALTHY" | "DEGRADED" | "UNHEALTHY" = "HEALTHY";
    if (successRate < 80) {
      status = "UNHEALTHY";
    } else if (successRate < 95 || fallbackRate > 0.2) {
      status = "DEGRADED";
    }
