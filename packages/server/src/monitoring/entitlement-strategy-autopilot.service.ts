...
import { createAutomationTask } from "../automation/notifications";
import {
...
function evaluateGuardrails(
  ...
  const calibrationWithinThreshold =
    calibrationGap === null || Math.abs(calibrationGap) <= thresholds.maxCalibrationGapAbs;
  ...
  return {
    ...
    recommendationEligible:
      kpiEligible && maeWithinThreshold && calibrationWithinThreshold && candidateEligible,
  };
}
...
export async function runEntitlementStrategyAutopilot(
  input: EntitlementStrategyAutopilotInput & { materializeTasks?: boolean },
