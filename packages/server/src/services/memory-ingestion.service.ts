...
export class EntityResolutionService {
  ...
  /**
   * Find fuzzy matches using PostgreSQL pg_trgm similarity().
   * Uses a GIN index on canonical_address — O(log n) instead of O(n) full scan.
   * The 0.3 floor pre-filters in SQL; the 0.85 threshold check remains in the caller.
   */
  private static async findFuzzyMatch(
    ...
    const results = await prisma.$queryRaw<Array<{
      ...
      SELECT id, "canonical_address",
             similarity("canonical_address", ${canonicalAddress}) AS similarity
      FROM "internal_entities"
      WHERE "org_id" = ${orgId}::uuid
        AND type = 'property'
        AND "canonical_address" IS NOT NULL
        AND similarity("canonical_address", ${canonicalAddress}) > 0.3
...
export class MemoryIngestionService {
  ...
  private static decomposeCompToFacts(comp: CompData): Array<{
    ...
    // Transaction parties
    if (comp.buyer || comp.seller) {
      facts.push({
        factType: 'transaction_parties',
        payload: { buyer: comp.buyer ?? null, seller: comp.seller ?? null },
      });
    }
