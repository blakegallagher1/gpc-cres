...
export function rankEpisodeResults(
  ...
  input: Pick<
    BuildLearningContextInput,
    "conversationId" | "dealId" | "jurisdictionId" | "runTypeHint"
  >,
  ...
  return [...results].sort((a, b) => {
    ...
    const aBoost =
      (input.dealId && am.dealId === input.dealId ? 0.3 : 0) +
      (input.jurisdictionId && am.jurisdictionId === input.jurisdictionId
        ? 0.2
        : 0) +
      (input.runTypeHint && am.taskType === input.runTypeHint ? 0.15 : 0);
    const bBoost =
      (input.dealId && bm.dealId === input.dealId ? 0.3 : 0) +
      (input.jurisdictionId && bm.jurisdictionId === input.jurisdictionId
        ? 0.2
        : 0) +
...
function rankProcedureResults(
  ...
  input: Pick<
    BuildLearningContextInput,
    "dealId" | "jurisdictionId" | "runTypeHint"
  >,
): KnowledgeSearchResult[] {
  return [...results].sort((a, b) => {
    const am = (a.metadata ?? {}) as Record<string, unknown>;
    const bm = (b.metadata ?? {}) as Record<string, unknown>;
    const aBoost =
      (input.dealId && am.dealId === input.dealId ? 0.2 : 0) +
      (input.jurisdictionId && am.jurisdictionId === input.jurisdictionId
        ? 0.15
        : 0) +
      (input.runTypeHint && am.taskType === input.runTypeHint ? 0.15 : 0);
    const bBoost =
      (input.dealId && bm.dealId === input.dealId ? 0.2 : 0) +
      (input.jurisdictionId && bm.jurisdictionId === input.jurisdictionId
        ? 0.15
        : 0) +
      (input.runTypeHint && bm.taskType === input.runTypeHint ? 0.15 : 0);
...
export async function buildLearningContext(
  ...
  const blocks: string[] = [];
  if (episodeResults.length > 0) blocks.push(formatEpisodeBlock(episodeResults));
  if (
    AUTOMATION_CONFIG.agentLearning.injectProcedures &&
