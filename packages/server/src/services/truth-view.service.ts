...
export async function getTruthView(
  ...
  try {
    ...
    const correctionRecords = verifiedRecords.filter((record) => record.factType === "correction");
    const nonCorrectionRecords = verifiedRecords.filter((record) => record.factType !== "correction");
    ...
    for (const correction of correctionRecords) {
      ...
      corrections.push({
        ...
        reason:
          typeof payload.correction_reason === "string" ? payload.correction_reason : "",
