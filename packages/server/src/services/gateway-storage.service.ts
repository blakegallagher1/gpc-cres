import { gatewayHeaders, getGatewayConfig } from "./gateway-proxy.service";

export type { GatewayAuth } from "./gateway-proxy.service";
import type { GatewayAuth } from "./gateway-proxy.service";
...
export async function getDownloadUrlFromGateway(
  ...
  if (!res.ok) {
    ...
    throw new Error(
      typeof data?.detail === "string"
        ? data.detail
        : data?.error || "Failed to get download URL",
    );
  }
