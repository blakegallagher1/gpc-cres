...
import { memoryWriteGate } from "./memory-write-gate.service";
...
function buildCompPrompt(line: string): string {
  return ["Comp addition or correction for the linked property/entity.", line].join("\n");
}

function buildLenderPrompt(line: string): string {
  return ["Lender term addition or correction for the linked property/entity.", line].join("\n");
}
...
function extractWhitelistedFactCandidates(outputJson: JsonRecord): CandidateFact[] {
  ...
  for (const line of lines) {
    ...
    if (
      ...
      candidate = { factClass: "comp", inputText: buildCompPrompt(line) };
    } else if (
      /\b(lender|ltv|dscr|rate bps|amortization|term months|recourse|prepayment)\b/i.test(line)
    ) {
      candidate = { factClass: "lender_term", inputText: buildLenderPrompt(line) };
    } else if (
    ...
    if (!candidate || seen.has(candidate.inputText)) continue;
...
async function resolveEntityContext(params: {
  ...
  const entity = await prisma.internalEntity.findFirst({
    where: {
      orgId: params.orgId,
      OR: [{ parcelId: firstParcel.id }, { canonicalAddress: firstParcel.address }],
    },
    select: {
      id: true,
      canonicalAddress: true,
...
export async function promoteCandidateFactsFromRun(
  ...
  if (input.status !== "succeeded" || !AUTOMATION_CONFIG.agentLearning.promoteFacts) {
    return { attempted: 0, verified: 0, drafted: 0, rejected: 0 };
  }
  ...
  const trustJson = isRecord(outputJson.trust) ? outputJson.trust : {};
  const missingEvidence = getStringArray(trustJson.missingEvidence);
  const confidence = extractConfidence(outputJson);

  if (
    confidence < AUTOMATION_CONFIG.agentLearning.minConfidenceForFactPromotion ||
    missingEvidence.length > 0
  ) {
    return { attempted: 0, verified: 0, drafted: 0, rejected: 0 };
  }

  const writeContext = await resolveEntityContext({
    orgId: input.orgId,
  ...
  for (const candidate of candidates) {
    throwIfAborted(input.signal);
    attempted++;
    const result = await memoryWriteGate(candidate.inputText, writeContext);
    if (result.decision === "verified") verified++;
    else if (result.decision === "draft") drafted++;
    else rejected++;
  }

  return { attempted, verified, drafted, rejected };
}
