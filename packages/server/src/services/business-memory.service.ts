...
const DOMAIN_KEYWORDS = [
  { domain: "strategy", keywords: ["moat", "strategy", "roadmap", "product", "vision", "growth", "business model", "positioning", "operating system"] },
  { domain: "operations", keywords: ["workflow", "process", "automation", "system", "ops", "sop", "task", "dashboard", "implementation", "team"] },
  { domain: "entitlement", keywords: ["entitlement", "zoning", "variance", "permit", "planning", "parish", "rezoning", "hearing", "commission", "site plan"] },
  { domain: "acquisition", keywords: ["acquisition", "seller", "owner", "purchase", "contract", "loi", "parcel", "site", "sourcing", "origination"] },
  { domain: "underwriting", keywords: ["underwriting", "pro forma", "rent", "expense", "noi", "cap rate", "yield", "irr", "dscr", "cash flow"] },
  { domain: "capital", keywords: ["capital", "equity", "debt", "loan", "lender", "financing", "refinance", "investor", "lp", "gp"] },
  { domain: "buyers", keywords: ["buyer", "disposition", "teaser", "outreach", "broker", "bid", "marketing process", "exit"] },
  { domain: "marketing", keywords: ["marketing", "brand", "website", "seo", "content", "email", "lead magnet", "campaign"] },
  { domain: "finance", keywords: ["budget", "tax", "cash", "burn", "accounting", "payroll", "expense", "bank"] },
  { domain: "hiring", keywords: ["hire", "hiring", "candidate", "employee", "recruiter", "talent", "interview"] },
  { domain: "relationships", keywords: ["planner", "attorney", "broker", "buyer", "lender", "partner", "city", "parish staff", "seller"] },
] as const;
...
function rankBusinessMemoryResults(
  ...
  return [...results].sort((left, right) => {
    ...
    const leftConversationBoost =
      context.conversationId && leftMetadata.conversationId === context.conversationId
        ? 0.3
        : 0;
    const rightConversationBoost =
      context.conversationId && rightMetadata.conversationId === context.conversationId
        ? 0.3
        : 0;
    const leftDealBoost =
      context.dealId && leftMetadata.dealId === context.dealId ? 0.2 : 0;
    const rightDealBoost =
      context.dealId && rightMetadata.dealId === context.dealId ? 0.2 : 0;
    return (
      right.similarity +
      rightConversationBoost +
      rightDealBoost -
...
function formatBusinessMemoryLine(result: KnowledgeSearchResult): string {
  ...
  const snippet = truncate(
    normalizeWhitespace(result.contentText),
...
async function searchBusinessMemory(
  ...
  try {
    ...
    return { contextBlock: "", results: [], retrievalMode: null };
  }
}
...
export async function buildBusinessMemoryContext(
  ...
  if (searchResult.results.length === 0) return searchResult;
