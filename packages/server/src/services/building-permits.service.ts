...
function getDatasetId(): string {
  return (
    process.env.SOCRATA_EBR_PERMITS_DATASET_ID ?? DEFAULT_SOCRATA_DATASET_ID
  );
}
...
async function fetchSocrataRows<T extends Record<string, unknown>>(
  ...
  try {
    ...
    if (error instanceof SocrataQueryError) throw error;
    throw new SocrataQueryError({
      queryName,
      message:
...
export async function getEbrBuildingPermitsFeed(
  ...
  const settledQueries = await Promise.allSettled(
    queries.map((query) =>
      fetchSocrataRows<Record<string, unknown>>(query.name, query.params),
    ),
  );
  ...
  for (const [index, settledQuery] of settledQueries.entries()) {
    ...
    if (query.required) requiredQueryFailed = true;
  }
  ...
  const totalsRow = (queryRows.get("totals") ?? [])[0] ?? {};
  const feed: BuildingPermitsFeed = {
    ...
    issuedTrend: (queryRows.get("issuedTrend") ?? []).map((row) => ({
    ...
    designationBreakdown: (queryRows.get("designationBreakdown") ?? []).map(
      (row) => ({
    ...
    topPermitTypes: (queryRows.get("topPermitTypes") ?? []).map((row) => ({
      label: parseNullableString(row.permit_type_label) ?? "Unknown",
      permitCount: parseNumber(row.permit_count),
      totalProjectValue: parseNumber(row.total_project_value),
    })),
    topZipCodes: (queryRows.get("topZipCodes") ?? []).map((row) => ({
      label: parseNullableString(row.zip) ?? "Unknown",
      permitCount: parseNumber(row.permit_count),
      totalProjectValue: parseNumber(row.total_project_value),
    })),
    recentPermits: (queryRows.get("recentPermits") ?? []).map((row) => ({
