...
export interface SentinelResult {
  ...
  warnCount: number;
  passCount: number;
  workflow: WorkflowStats | string;
  probes: Record<string, Array<{ status: number; ms: number }>>;
  evaluatedAt: string;
}
...
export function getBaseUrl(): string {
  return (process.env.BASE_URL ?? "https://gallagherpropco.com").replace(/\/$/, "");
}

export function getProductionMode(): boolean {
  const BASE = getBaseUrl();
  return process.env.SENTINEL_PRODUCTION_MODE === "false"
    ? false
...
export function statusCounts(runs: ProbeRun[]) {
  let auth = 0;
  let s405 = 0;
  let s429 = 0;
  let s5xx = 0;
  ...
  return { total: runs.length, auth, s405, s429, s5xx };
}

export function latencyEligibleMs(runs: ProbeRun[]): number[] {
  return runs
    .filter((run) => run.status >= 200 && run.status < 400)
    .map((run) => run.totalMs);
}

export function isSelfHostedSentinelWebhook(webhookUrl: string): boolean {
  try {
    const parsedWebhook = new URL(webhookUrl);
    const parsedBase = new URL(getBaseUrl());
    return (
...
// ---------------------------------------------------------------------------
// Workflow stats via Prisma
// ---------------------------------------------------------------------------

export async function queryWorkflowStats(): Promise<WorkflowStats | null> {
  try {
...
export function evaluate(
  ...
  const THRESHOLDS = getThresholds();
  const PRODUCTION_MODE = getProductionMode();
...
export async function runStabilitySentinel(): Promise<SentinelResult> {
  const THRESHOLDS = getThresholds();
  const BASE = getBaseUrl();
  const GEOMETRY_PARCEL_ID =
  ...
  if (verdict === "FAIL") {
    await dispatchAlert(result as unknown as Record<string, unknown>, checks);
  }
