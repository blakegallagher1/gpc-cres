...
export type SourceIngestionAlertConfig = {
...
export type SourceIngestionAlertDecision = {
  shouldSend: boolean;
...
export type SourceIngestionAlertCandidate = {
  orgId: string;
  runId: string;
  staleRatio: number;
  staleCount: number;
...
export type StaleSourceOffender = {
  url: string;
...
export function getSourceIngestionAlertConfig(): SourceIngestionAlertConfig {
  return {
...
function formatOffenderLine(entry: StaleSourceOffender): string {
...
function parseSourceIngestionAlertMetadata(
  ...
  if (staleSourceManifestHash === null || offenderSignature === null) return null;

  return {
    alertTag: typeof value.sourceIngestAlertTag === "string"
      ? value.sourceIngestAlertTag
      : SOURCE_INGEST_ALERT_TAG,
    staleOffenderCount: Number.isFinite(staleOffenderCount) ? staleOffenderCount : 0,
...
async function getSourceIngestionAlertDecision(
  ...
  const cutoff = new Date(now.getTime() - config.dedupeWindowHours * 3_600_000);
  const recentAlerts = await getRecentSourceIngestionAlertRecords(candidate.orgId, cutoff);
  ...
  for (const pack of packs) {
    ...
    for (const candidate of urls) {
      ...
      try {
        ...
        logger.warn("Cron source-ingestion failed to discover source", {
          url: normalized,
...
export async function runSourceIngestion(): Promise<SourceIngestionSummary> {
  ...
  // --- Build per-org summaries, persist run records, dispatch alerts ---
