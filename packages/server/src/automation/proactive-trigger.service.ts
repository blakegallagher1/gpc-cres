...
import { getStructuredPreferences } from "../services/preference.service";
import {
...
function conditionMatches(
  ...
  const eventValue =
    directValue === undefined ? getNestedValue(payload, `payload.${condition.field}`) : directValue;
...
async function computeMatchConfidence(
  ...
  if (Array.isArray(preferredJurisdictions) && typeof jurisdiction === "string") {
    if (preferredJurisdictions.map(String).includes(jurisdiction)) {
      confidence += 0.1;
    }
  }
  ...
  if (Array.isArray(preferredSkus) && typeof sku === "string") {
    if (preferredSkus.map(String).includes(sku)) {
      confidence += 0.1;
    }
  }
...
export async function evaluateProactiveEvent(params: {
  ...
  for (const trigger of triggers) {
    ...
    if (expectedEvent && expectedEvent !== params.eventType) {
      continue;
    }
    ...
    if (!allConditionsMatch(conditions, params.payload)) {
      continue;
    }
    ...
    if (runsToday >= trigger.maxRunsPerDay) {
      continue;
    }

    const confidence = await computeMatchConfidence(params.orgId, trigger.createdBy, params.payload);
    if (confidence < 0.5) {
      continue;
    }
