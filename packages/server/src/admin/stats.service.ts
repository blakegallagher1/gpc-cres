...
export type AdminTabError = {
  message: string;
  detail?: string;
};

export type AdminTabErrors = Record<string, AdminTabError>;
...
export const VALID_TABS = ["overview", "knowledge", "memory", "agents", "system", "all"] as const;

// ---------------------------------------------------------------------------
// Schema drift detection (duplicated from apps/web — pure function, no server-only imports)
// ---------------------------------------------------------------------------
...
async function runTabSafely<T>(
  tab: string,
  errors: AdminTabErrors,
  runner: () => Promise<T>,
): Promise<T | null> {
  try {
    return await runner();
...
async function runCountWithFallback(
  orgId: string,
  ...
  const modelMap: Record<string, () => Promise<number>> = {
    run: () => prisma.run.count({ where: { orgId } }),
    trajectoryLog: () => prisma.trajectoryLog.count({ where: { orgId } }),
    episodicEntry: () => prisma.episodicEntry.count({ where: { orgId } }),
    proceduralSkill: () => prisma.proceduralSkill.count({ where: { orgId } }),
    memoryVerified: () => prisma.memoryVerified.count({ where: { orgId } }),
    internalEntity: () => prisma.internalEntity.count({ where: { orgId } }),
    deal: () => prisma.deal.count({ where: { orgId } }),
    conversation: () => prisma.conversation.count({ where: { orgId } }),
  };

  const runner = modelMap[model as keyof typeof modelMap];
  if (!runner) return 0;
  return (await runTabSafely(tab, errors, runner)) ?? 0;
}

async function safeEpisodeCount(
  orgId: string,
  tab: string,
  errors: AdminTabErrors,
) {
  return (
    (await runTabSafely(tab, errors, () =>
...
export async function getAdminStats(
  ...
  const { tab, page, limit, offset, search, contentType, subTab } = params;
  ...
  if (shouldLoadOverview) {
    ...
    const [
      ...
      runTabSafely("overview", errors, async () => {
      ...
      runTabSafely("overview", errors, () =>
        prisma
          .$queryRawUnsafe<Array<{ content_type: string; count: bigint }>>(
    ...
    result.overview = {
      ...
      promotionBreakdown: (promotionGroups ?? []).reduce<
  ...
  if (shouldLoadKnowledge) {
    ...
    const paramsArr: unknown[] = [orgId, limit, offset];
    ...
    const [rows, totalCount, contentTypes] = await Promise.all([
      runTabSafely<KnowledgeEmbeddingRow[]>("knowledge", errors, () =>
      ...
      runTabSafely<number>("knowledge", errors, () =>
        prisma
          .$queryRawUnsafe<[{ count: bigint }]>(
      ...
      runTabSafely<string[]>("knowledge", errors, () =>
        prisma
          .$queryRawUnsafe<KnowledgeContentTypeRow[]>(
      ...
      const [rows, total] = await Promise.all([
        runTabSafely("memory", errors, () =>
          prisma.memoryVerified.findMany({
            ...
            include: {
              entity: {
                select: { id: true, canonicalAddress: true, type: true },
              },
            },
          }),
        ),
  ...
  if (shouldLoadAgents) {
    ...
    const [runs, total, stats, dailyByRunType, learningCounts] =
      await Promise.all([
        runTabSafely("agents", errors, () =>
        ...
        runTabSafely("agents", errors, async () => {
          ...
          return {
            ...
            successRate:
              aggregate._count > 0 && succeeded
                ? Math.round((succeeded / aggregate._count) * 100)
                : 0,
          };
        }),
        runTabSafely("agents", errors, () =>
