...
async function buildArtifactSpec(
  ...
  switch (artifactType) {
    ...
    case "IC_DECK_PPTX": {
      const t = triageOutput!;
      const ctx = buildDealContext(deal, t);
      const sys = "You are a senior CRE investment analyst at Gallagher Property Company preparing an Investment Committee presentation. Write concise bullet points (not paragraphs). Each bullet should be a single key fact, metric, or recommendation. Use available deal data — never fabricate numbers.";
        ...
        slides: [
          {
            ...
            bullets: [
              ...
              `Location: ${deal.jurisdiction?.name ?? "Louisiana"}`,
              `Total Acreage: ${acreage} acres`,
              `Triage: ${String(t.decision ?? "N/A")} (${String(t.confidence ?? "N/A")})`,
            ],
            speaker_notes: "Introduce the deal to the Investment Committee.",
          },
          {
            slide_no: 2,
            title: "Site & Property",
            ...
            speaker_notes: "Walk through the site details and current entitlement status.",
          },
          {
            slide_no: 3,
            title: "Market Context",
            bullets: parseBullets(marketBullets),
            speaker_notes: "Present the market analysis supporting this investment thesis.",
          },
          {
            slide_no: 4,
            title: "Financial Projections",
            bullets: parseBullets(financialBullets),
            speaker_notes: "Walk through financial model assumptions and projected returns.",
          },
          {
            slide_no: 5,
            title: "Risk Assessment",
            bullets: parseBullets(riskBullets),
            speaker_notes: "Address each key risk area and the proposed mitigation strategy.",
          },
          {
            slide_no: 6,
            title: "Development Plan",
            bullets: parseBullets(planBullets),
            speaker_notes: "Outline the execution plan from closing through stabilization.",
          },
          {
            slide_no: 7,
            title: "Recommendation & Vote",
            bullets: parseBullets(recBullets),
            speaker_notes: "Present the recommendation and call for the IC vote.",
          },
        ],
        sections: [],
      };
