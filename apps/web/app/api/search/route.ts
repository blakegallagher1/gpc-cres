...
import {
  ...
  type GlobalSearchConversationResult,
  type GlobalSearchDealResult,
  type GlobalSearchKnowledgeResult,
  type GlobalSearchParcelResult,
  type GlobalSearchResponse,
  type GlobalSearchRunResult,
  type GlobalSearchSource,
} from "@/lib/search/globalSearch";
import { searchKnowledgeBase } from "@/lib/services/knowledgeBase.service";
import {
...
export async function GET(request: NextRequest) {
  ...
  const response = createEmptyResponse(query, limit);

  try {
    const [deals, parcels, knowledge, runs, conversations] = await Promise.allSettled([
      searchDeals(auth.orgId, query, limit),
      searchParcels(query, limit),
      searchKnowledge(auth.orgId, query, limit),
      searchRuns(auth.orgId, query, limit),
      searchConversations(auth.orgId, query, limit),
    ]);
