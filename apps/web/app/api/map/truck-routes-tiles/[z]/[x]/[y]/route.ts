...
/**
 * Truck routes vector tile proxy.
 *
 * Tile upstream is configured via TRUCK_ROUTES_TILE_ORIGIN. When the env var
 ...
 *
 * Path template: `${TRUCK_ROUTES_TILE_ORIGIN}${TRUCK_ROUTES_TILE_PATH}` with
 * `{z}`, `{x}`, `{y}` substitutions. `TRUCK_ROUTES_TILE_PATH` defaults to
 * `/truck_routes/{z}/{x}/{y}`.
 */
export async function GET(_req: Request, { params }: RouteParams) {
  ...
  const origin = process.env.TRUCK_ROUTES_TILE_ORIGIN?.trim();
  if (!origin) {
  ...
  const pathTemplate = process.env.TRUCK_ROUTES_TILE_PATH?.trim() || "/truck_routes/{z}/{x}/{y}";
  ...
  const token = process.env.TRUCK_ROUTES_TILE_TOKEN?.trim();
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Truck routes tile upstream returned ${response.status}`);
    }
    ...
    Sentry.captureException(err, {
      tags: { route: "api.map.truck-routes-tiles", method: "GET", timeout: String(isTimeout) },
    });
    return NextResponse.json(
      { error: isTimeout ? "Truck routes tile request timed out" : "Failed to fetch truck routes tile" },
