...
async function handleRequest(request: NextRequest) {
  ...
  return NextResponse.json(
    mapWorkspaceService.buildAssemblageSnapshot(workspace, context),
  );
}
...
export async function GET(request: NextRequest) {
  try {
    ...
    Sentry.captureException(error, {
      tags: { route: "api.map.assemblage", method: "GET" },
    });
    ...
    return NextResponse.json(
      { error: "Failed to load assemblage analysis" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    Sentry.captureException(error, {
      tags: { route: "api.map.assemblage", method: "POST" },
    });
