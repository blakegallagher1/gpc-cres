...
/**
 * Major interchanges vector tile proxy.
 *
 * Tile upstream is configured via INTERCHANGES_TILE_ORIGIN. When the env var
 * is unset, this route returns 204 so the layer renders nothing — the UX is
 * ready but the data plane can be wired later without code changes.
 *
 * Path template: `${INTERCHANGES_TILE_ORIGIN}${INTERCHANGES_TILE_PATH}` with
 * `{z}`, `{x}`, `{y}` substitutions. `INTERCHANGES_TILE_PATH` defaults to
 * `/interchanges/{z}/{x}/{y}`.
 */
export async function GET(_req: Request, { params }: RouteParams) {
  ...
  const origin = process.env.INTERCHANGES_TILE_ORIGIN?.trim();
  if (!origin) {
  ...
  const pathTemplate = process.env.INTERCHANGES_TILE_PATH?.trim() || "/interchanges/{z}/{x}/{y}";
  ...
  const token = process.env.INTERCHANGES_TILE_TOKEN?.trim();
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Interchanges tile upstream returned ${response.status}`);
    }
    ...
    Sentry.captureException(err, {
      tags: { route: "api.map.interchanges-tiles", method: "GET", timeout: String(isTimeout) },
    });
    return NextResponse.json(
      { error: isTimeout ? "Interchanges tile request timed out" : "Failed to fetch interchanges tile" },
