...
/**
 * Freight rail vector tile proxy.
 *
 * Tile upstream is configured via RAIL_TILE_ORIGIN. When the env var is
 ...
 *
 * Path template: `${RAIL_TILE_ORIGIN}${RAIL_TILE_PATH}` with `{z}`, `{x}`,
 * `{y}` substitutions. `RAIL_TILE_PATH` defaults to `/rail/{z}/{x}/{y}`.
 */
export async function GET(_req: Request, { params }: RouteParams) {
  ...
  const origin = process.env.RAIL_TILE_ORIGIN?.trim();
  if (!origin) {
  ...
  const pathTemplate = process.env.RAIL_TILE_PATH?.trim() || "/rail/{z}/{x}/{y}";
  ...
  const token = process.env.RAIL_TILE_TOKEN?.trim();
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Rail tile upstream returned ${response.status}`);
    }
    ...
    Sentry.captureException(err, {
      tags: { route: "api.map.rail-tiles", method: "GET", timeout: String(isTimeout) },
    });
    return NextResponse.json(
      { error: isTimeout ? "Rail tile request timed out" : "Failed to fetch rail tile" },
