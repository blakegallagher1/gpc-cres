...
export function buildOpportunityThesis(
  ...
  if (parishWeight > 0.4) {
    whyNowParts.push(`Your recent pursued opportunities cluster in ${parish}, so the engine is giving this market extra weight.`);
  } else if (matchedSavedSearch) {
    whyNowParts.push("The parcel is aligned with one or more explicit saved-search criteria, so it cleared the first-pass filter without manual triage.");
  } else {
    whyNowParts.push("The parcel cleared the baseline saved-search filter and surfaced as a fresh candidate for review.");
  }
  ...
  if (!(match.parcelData.lat && match.parcelData.lng)) {
    keyRisks.push("Parcel coordinates are missing, so map/comps diligence is incomplete until geometry is verified.");
  }
  ...
  if (!matchedSavedSearch) {
    keyRisks.push("Matched-criteria evidence is thin, so this may be a broader saved-search hit rather than a tightly qualified parcel.");
  }
  if (keyRisks.length === 0) {
    keyRisks.push("No material structural risk surfaced from the cached parcel snapshot; validate with comps and geometry next.");
  }
  ...
  if (!(match.parcelData.lat && match.parcelData.lng)) {
    ...
    nextBestAction = "Create or continue the deal record now, then run comps and parcel geometry review while the thesis is fresh.";
  } else if (parishWeight > 0.4) {
    nextBestAction = `Compare this parcel against other pursued ${parish} opportunities before deciding whether to escalate.`;
  }

  const confidence = round2(
    clamp(priorityScore / 100 * 0.65 + profile.confidence * 0.35, 0, 1),
  );
