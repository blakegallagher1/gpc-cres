...
import { computeEffectiveWeight } from "./calibrationDecay";
import { CALIBRATION_MINIMUMS } from "./calibrationConfig";
...
function combineProjectionSnapshots(
  ...
  for (const record of records) {
    ...
    for (const [metricKey, projectedValue] of Object.entries(metric)) {
      if (!(metricKey in merged)) {
        merged[metricKey] = projectedValue;
      }
    }
  }

  // Snapshot values (captured at deal close) win over current memory.
  for (const [metricKey, value] of Object.entries(parseJsonObject(projectionSnapshot))) {
    const numericValue = asNumber(value);
    if (numericValue !== null) {
      merged[metricKey] = numericValue;
    }
  }
...
async function resolveSegmentDescriptor(orgId: string, entityId: string): Promise<CalibrationSegmentDescriptor> {
  ...
  // Defaults until we have canonical deal/segment metadata.
  const strategy = "default";
  const leverageBand = "default";
  const vintageYear = entity?.createdAt ? entity.createdAt.getUTCFullYear() : new Date().getUTCFullYear();
  ...
  const addressPart = typeof entity.canonicalAddress === "string" && entity.canonicalAddress.length > 0
...
export async function ingestOutcome(
  ...
  const projections = combineProjectionSnapshots(
    projectionSnapshot,
    verifiedProjectionRecords.map((r) => ({ payloadJson: r.payloadJson })),
  );
  const finals = combineFinalMetrics(finalMetrics);

  const targetKeys = Object.keys(finals);
  ...
  const entries = Object.entries(finals)
    .map(([metricKey, actualValue]) => {
      ...
      if (projectedValue === null || projectedValue === undefined) {
        return null;
      }
      return {
