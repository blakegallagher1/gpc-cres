import "server-only";
import { addMarketDataPoint } from "./marketMonitor.service";

// ---------------------------------------------------------------------------
// Louisiana city → parish mapping (GPC primary + secondary markets)
// ---------------------------------------------------------------------------
...
// ZIP prefix → parish for fallback (first 3 digits of 5-digit ZIP)
...
// ---------------------------------------------------------------------------
// Address → parish extraction
// ---------------------------------------------------------------------------

/**
 * Extract Louisiana parish from an address string.
 * Tries city name match first, then ZIP prefix, then returns null.
 */
export function extractParishFromAddress(address: string | null | undefined): string | null {
  if (!address) return null;

  // Try to extract city: look for "City, LA" or "City, Louisiana" pattern
...
// ---------------------------------------------------------------------------
// Bridge: verified comp → MarketDataPoint
// ---------------------------------------------------------------------------
...
/**
 * Bridge a verified comp write into the MarketDataPoint table.
 * Fire-and-forget — never blocks the write route response.
 */
export function bridgeCompToMarket(
  ...
  // Resolve parish: address → market field → default
  const parish =
  ...
  // Build the data blob matching what getParishSummary/getMarketTrends read
  ...
  // observedAt: prefer sale_date if available, otherwise now
