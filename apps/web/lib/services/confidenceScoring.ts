...
import { MIN_CALIBRATION_SAMPLES } from "./promotionConfig";

/**
 * 5-dimensional confidence vector stored on each memory object.
 */
...
/**
 * Structural confidence: measures payload completeness.
 * Counts non-null, non-empty fields as a fraction of total fields.
 */
...
/**
 * Source reliability: looks up MemorySourceRegistry for the source.
 * Falls back to default based on sourceType.
 */
async function computeSourceReliability(
  orgId: string,
  sourceType: string,
): Promise<number> {
...
/**
 * Cross-memory agreement: checks how many existing verified records
 * for the same entity+factType have overlapping payload keys with
 * consistent values. Higher agreement = more consistent with existing memory.
 */
...
/**
 * Calibration support: measures whether the calibration segment for this entity
 * has enough data and low enough variance to support confidence.
 */
async function computeCalibrationSupport(
  orgId: string,
  entityId: string,
): Promise<number> {
...
/**
 * Compute the full 5-dimensional confidence vector for a memory write.
 */
...
/**
 * Parse a stored confidence vector from JSON, with fallback to defaults.
 */
export function parseConfidenceVector(json: unknown): ConfidenceVector {
  ...
  return {
    structural_confidence: typeof obj.structural_confidence === "number" ? obj.structural_confidence : 0.5,
    source_reliability_score: typeof obj.source_reliability_score === "number" ? obj.source_reliability_score : 0.5,
    cross_memory_agreement_score: typeof obj.cross_memory_agreement_score === "number" ? obj.cross_memory_agreement_score : 0.5,
    calibration_support_score: typeof obj.calibration_support_score === "number" ? obj.calibration_support_score : 0.5,
