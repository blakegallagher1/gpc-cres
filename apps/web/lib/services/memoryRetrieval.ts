...
function getFactTypeFilter(intent: IntentClassification["intent"]): string[] {
...
function buildRequiredFilters(
  ...
  const andFilters = entries.map(([key, value]) => ({
    payloadJson: {
      path: [key],
      equals: value,
    },
...
function itemTokenEstimate(items: ScoredRecord[]): number {
  const payload = items
    .map((item) => JSON.stringify(item.record.payloadJson))
    .join("");
...
function trimFromTier2(
  ...
  while (
    itemTokenEstimate([...tier0Items, ...tier1Items, ...tier2Items]) > totalBudget
  ) {
...
export async function retrieveMemoryForIntent(
  ...
  return {
    ...
    tier1Items,
    tier2Items,
    totalTokensEstimate: itemTokenEstimate([
      ...tier0Items,
      ...tier1Items,
      ...tier2Items,
    ]),
