...
export const STAGE_TRANSITIONS: ReadonlyArray<{
...
}> = [
  {
    ...
    pipelineStep: 2,
    criteriaDescription:
      "All Step 2 tasks completed and triage decision is ADVANCE",
  },
  {
    from: "PREAPP",
    to: "CONCEPT",
    pipelineStep: 3,
    criteriaDescription:
      "Pre-app meeting notes uploaded and all Step 3 tasks completed",
  },
  {
    from: "CONCEPT",
    to: "NEIGHBORS",
    pipelineStep: 4,
    criteriaDescription: "Concept plan uploaded and site plan approved",
  },
  {
    from: "NEIGHBORS",
    to: "SUBMITTED",
    pipelineStep: 5,
    criteriaDescription:
      "Neighbor notification complete and no unresolved objections",
  },
  {
    from: "SUBMITTED",
    to: "HEARING",
    pipelineStep: 6,
    criteriaDescription: "Application submitted and hearing date set",
  },
  {
    from: "HEARING",
    to: "APPROVED",
    pipelineStep: 7,
    criteriaDescription: "Hearing outcome approved",
  },
  {
    from: "APPROVED",
    to: "EXIT_MARKETED",
    pipelineStep: 8,
    criteriaDescription: "Exit package generated and deal listed for sale",
  },
];

export const NON_TASK_DRIVEN_STAGE_KEYS = new Set([
  "ORIGINATION",
  "DISPOSITION",
  "CLOSED_WON",
  "CLOSED_LOST",
...
export function getNextTransition(
  ...
  return STAGE_TRANSITIONS.find((transition) => transition.from === status) ?? null;
}

export function describeWorkflowAdvanceCriteria(
...
export async function handleAdvancement(event: AutomationEvent): Promise<void> {
