...
async function hasRecentTask(
  ...
  const cutoff = new Date(
    Date.now() -
      AUTOMATION_CONFIG.marketMonitoring.dedupeWindowHours * 60 * 60 * 1000,
  );
...
export async function runMarketMonitoring(
  now = new Date(),
  ...
  const currentWindowStart = new Date(
    now.getTime() -
      AUTOMATION_CONFIG.marketMonitoring.lookbackDays * 24 * 60 * 60 * 1000,
  );
  ...
  for (const [parish, parishDeals] of dealsByParish.entries()) {
    ...
    if (
      ...
      Math.abs(capRateDeltaBps) >=
        AUTOMATION_CONFIG.marketMonitoring.capRateShiftBps
    ) {
  ...
  if (
    ...
    Math.abs(roundedPortfolioShiftBps) >=
      AUTOMATION_CONFIG.marketMonitoring.rateEnvironmentShiftBps
  ) {
    dscrAlertsCreated = await createDscrRecalcTasks(
      activeDeals,
      roundedPortfolioShiftBps,
    );
  }
