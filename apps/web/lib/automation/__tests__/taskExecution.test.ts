jest.mock("@entitlement-os/db", () => ({
  prisma: {
    task: { findFirst: jest.fn(), count: jest.fn(), create: jest.fn() },
  },
}));

const db = jest.requireMock("@entitlement-os/db") as {
  prisma: {
    task: { findFirst: jest.Mock; count: jest.Mock; create: jest.Mock };
  };
};
...
describe("handleTaskCreated", () => {
  ...
  it("returns if task status is not TODO", async () => {
    db.prisma.task.findFirst.mockResolvedValue({ id: "t", title: "Check flood zone", status: "IN_PROGRESS" });
    await handleTaskCreated({ type: "task.created", dealId: "d", taskId: "t", orgId: "o" });
    expect(db.prisma.task.count).not.toHaveBeenCalled();
  });

  it("skips [AUTO] tasks", async () => {
    db.prisma.task.findFirst.mockResolvedValue({ id: "t", title: "[AUTO] Ready for triage", status: "TODO" });
    await handleTaskCreated({ type: "task.created", dealId: "d", taskId: "t", orgId: "o" });
    expect(db.prisma.task.count).not.toHaveBeenCalled();
  });

  it("returns for human-only tasks (e.g. 'call the city')", async () => {
    db.prisma.task.findFirst.mockResolvedValue({ id: "t", title: "Call the city planner", status: "TODO" });
    await handleTaskCreated({ type: "task.created", dealId: "d", taskId: "t", orgId: "o" });
  ...
  it("returns when concurrent task limit reached", async () => {
    db.prisma.task.findFirst.mockResolvedValue({ id: "t", title: "Check flood zone data", status: "TODO" });
    db.prisma.task.count.mockResolvedValue(5); // >= maxConcurrentPerDeal (5)
    await handleTaskCreated({ type: "task.created", dealId: "d", taskId: "t", orgId: "o" });
    // Should log but not fail
    expect(db.prisma.task.count).toHaveBeenCalled();
  });

  it("identifies agent-executable task under concurrent limit", async () => {
    db.prisma.task.findFirst.mockResolvedValue({ id: "t", title: "Check flood zone data", status: "TODO" });
    db.prisma.task.count.mockResolvedValue(2); // < maxConcurrentPerDeal (5)
...
describe("handleTaskCompleted", () => {
  ...
  it("skips quality check if task is not DONE", async () => {
    db.prisma.task.findFirst.mockResolvedValue({ id: "t", status: "TODO", description: "Short" });
    await handleTaskCompleted({ type: "task.completed", dealId: "d", taskId: "t", orgId: "o" });
    expect(db.prisma.task.create).not.toHaveBeenCalled();
  });

  it("skips quality check if no description", async () => {
    db.prisma.task.findFirst.mockResolvedValue({ id: "t", status: "DONE", description: null });
    await handleTaskCompleted({ type: "task.completed", dealId: "d", taskId: "t", orgId: "o" });
    expect(db.prisma.task.create).not.toHaveBeenCalled();
  });

  it("skips quality check if no Agent Findings section", async () => {
    db.prisma.task.findFirst.mockResolvedValue({
  ...
  it("creates review task when agent findings too short", async () => {
    ...
    db.prisma.task.create.mockResolvedValue({ id: "review-task" });
