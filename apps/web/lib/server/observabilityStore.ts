...
function stripIdentityValue(value: unknown): unknown {
  ...
  if (Array.isArray(value)) {
    return value.map((entry) =>
      typeof entry === "object" && entry !== null ? stripIdentityFields(entry as Record<string, unknown>) : entry,
    );
  }
  if (typeof value === "object") {
...
export function queryObservabilityStore(options: ObservabilityQueryOptions = {}): ObservabilityQueryResult {
  const store = getStore();
  ...
  const events = normalized.kind === "monitor"
    ? []
    : store.events
        ...
        .filter((entry) => (normalized.status ? String(entry.status ?? "") === normalized.status : true))
        .slice(0, normalized.limit);

  const monitorSnapshots = normalized.kind === "event"
    ? []
    : store.monitorSnapshots
        .filter((entry) => matchesCommonFilters(entry, normalized))
        .filter((entry) => (normalized.status ? toLower(entry.status) === toLower(normalized.status) : true))
