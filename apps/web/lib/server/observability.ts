...
function writeLog(level: ObservabilityLevel, message: string): void {
  switch (level) {
    case "debug":
...
function toMonitorStatus(value: unknown): string {
  return typeof value === "string" ? value : "unknown";
}

function mapDurableStatus(kind: "event" | "monitor", status: number | string | null, level?: ObservabilityLevel): string {
  if (kind === "monitor") {
...
async function persistDurableObservabilityEvent(
  kind: "event" | "monitor",
...
function normalizeDurableEvent(row: DurableAutomationEventRecord): ObservabilityEventRecord | null {
...
function normalizeDurableMonitor(row: DurableAutomationEventRecord): ObservabilityMonitorSnapshot | null {
  const input = asRecord(row.inputData);
  const output = asRecord(row.outputData);
...
function mergeUniqueById<T extends { id: string; recordedAtMs: number }>(
  primary: T[],
  secondary: T[],
  limit: number,
): T[] {
...
function filterDurableEvents(events: ObservabilityEventRecord[], options: ObservabilityQueryOptions): ObservabilityEventRecord[] {
...
function filterDurableMonitors(
...
async function queryDurableObservability(
  options: ObservabilityQueryOptions,
