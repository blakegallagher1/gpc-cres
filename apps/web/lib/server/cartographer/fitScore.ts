...
export async function computeFitScores(
  ...
  const response = await gateway.query(validation.sanitizedSql);
  const rows: Array<Record<string, unknown>> =
    Array.isArray(response) ? response : (response as { rows?: unknown[] }).rows ?? [];
  ...
  // ---- Build heatmap layer ----
  const features: GeoJsonFeature[] = scores
    .map((s, idx) => {
      const row = rows[idx];
...
function scoreParcel(
  ...
  for (const entry of thesis.weights) {
    const rawValue = row[entry.factor];
