...
export async function findAssemblage(
  ...
  try {
    const response = await gateway.query(clusterSql);
    rows = Array.isArray(response) ? response : (response as { rows?: unknown[] }).rows ?? [];
  } catch {
    // Fallback: just run the candidate SQL directly, skip clustering
    const fallbackResponse = await gateway.query(validation.sanitizedSql);
    rows = Array.isArray(fallbackResponse)
      ? fallbackResponse
      : (fallbackResponse as { rows?: unknown[] }).rows ?? [];
